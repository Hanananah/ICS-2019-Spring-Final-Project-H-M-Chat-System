...
class ClientSM:
    ...
    def game(self,peer):
        ...
        if response["status"] == "success":
            ...
            self.game_grf = response["game_grf"]
            self.out_msg += 'Playing game with ' + peer + ' now! Have Fun!\n\n'
            self.out_msg += '-----------------------------------\n'
            self.out_msg += "Current Situation:\n"

            self.out_msg += self.prt(response["game_grf"]) + '\n'
            self.out_msg += response["order"]
        ...
        if self.state == S_LOGGEDIN:
            ...
            if len(peer_msg) > 0:
                ...
                if peer_msg["action"] == "game_request":
                    ...
                    self.out_msg += '-----------------------------------\n'
                    self.out_msg += "Current Situation:\n"
                    self.out_msg += self.prt(peer_msg["game_grf"]) + '\n'
                    self.out_msg += peer_msg["order"]
                    self.game_grf = peer_msg["game_grf"]
                    self.color = peer_msg["color"]
                    self.state = S_GAMING
...
# ==============================================================================
            ...
            if len(my_msg) > 0:
                ...
                if msg.isdigit():
                    for i in range(3):
                        for j in range(3):
                            if self.game_grf[i][j] == int(msg):
                                self.game_grf[i][j] = self.color
                print(self.prt(self.game_grf))
                mysend(self.s,json.dumps({"action":"game_exchange", "from":"[" + self.me + "]", "game_grf":self.game_grf}))
            if len(peer_msg) > 0:
                peer_msg = json.loads(peer_msg)
                if peer_msg["status"] == "finish":
                    self.state = S_LOGGEDIN
                    self.out_msg = peer_msg["message"]
                    self.out_msg += self.prt(peer_msg["game_grf"])
                    self.out_msg += "The game is finished, you are alone\n"
                    self.color = ''
                    self.game_grf = []
